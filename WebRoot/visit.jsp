...
<div id="sidebarcenter">
  ...
  <div id="content">
    ...
    <div id="main-left">
      <!--被访问者信息-->
      <logic:present name="vumspn" scope="session">
      <bean:define id="myvUser" name="vumspn" scope="session" property="user"></bean:define>
      </logic:present>
      <div id="vuser-info">
        <div id="vimg"><img src='${myvUser.uImg}' width="250" height="250" /></div>
        <div id="vsumit">
          <div>
            <h2>${myvUser.uName }的微博</h2>
          </div>
          <ul>
            <li>积分：${myvUser.jNum.num}</li>
            <li>等级：待定</li>
            <li>粉丝：<span id="vfowednum">${myvUser.fowedNum }</span></li>
            <li>微博：<span id="vmnum">${myvUser.mNum }</span></li>
          </ul>
          <div id="vuser-jj">简介：${myvUser.uc.uSyn }</div>
          <div id="vfollow">
          <logic:notPresent name="umspn" scope="session">
          	<a href="javascript:void(0)" onClick="dodenlu()">加关注</a>
          	<a href="javascript:void(0)" onClick="dodenlu()">取消关注</a>
          </logic:notPresent>
          <logic:present name="umspn" scope="session">
          	<logic:notEqual value="${myvUser.uId}" name="umspn" property="user.uId" scope="session"><!-- 自己不能关注自己 -->
            <logic:present name="havefollow" scope="request"><!-- 表明已经关注了 -->
            <a href="javascript:void(0)" style="background-color:#CCC;">加关注</a>
            <a href="javascript:void(0)" onClick="doUnFollow('${myUser.uId}','${myvUser.uId }')">取消关注</a>
            </logic:present>
            <logic:notPresent name="havefollow" scope="request">
            <a href="javascript:void(0)" onClick="doFollow('${myUser.uId}','${myvUser.uId }')">加关注</a>
      ...
      <div id="oldcon">
				<logic:present name="vumspn" scope="session">
				<logic:notEmpty name="vumspn" scope="session">
				<logic:notEmpty name="vumspn" scope="session" property="msgSet">
				<bean:define id="vmyMsgSet" name="vumspn" scope="session" property="msgSet"></bean:define>
				<logic:iterate id="myMsg" name="vmyMsgSet">
					...
					<logic:present name="vumspn" scope="session">
					<logic:greaterThan value="1" name="vumspn" property="pageNum" scope="session">
					<div>
					<a href="getNextv.do?type=up&page=
					...
					">上页</a>
					&nbsp;&nbsp;
					<a href="getNextv.do?type=down&page=
					<logic:present name="page" scope="session">
					<bean:write name="page" scope="session"/>
					</logic:present>
					<logic:notPresent name="page" scope="session">1</logic:notPresent>
					...
					<logic:present name="pageTishiv" scope="request">
					<bean:write name="pageTishiv" scope="request"/>
					</logic:present>
				</div>
